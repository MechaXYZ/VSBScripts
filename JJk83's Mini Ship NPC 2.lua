...
function Ready()
	...
	FakeChar.Name = "NPC2"..tostring(Spawned)
	Hint.Text = [[




Set 2 NPC: ]]..Color.Name.." / Total: "..Spawned
...
Set 2 NPC: ]]..Color.Name.." / Health: "..tostring(HP).." / Total: "..Spawned
			Show = Show + 1
		end)
	end
...
Set 2 NPC: ]]..Color.Name.." / Health: "..tostring(HP).." / Total: "..Spawned
	...
	if a ~= "That really hit!" or math.random(1,8) == 1 then
		...
		H.Text = a.." 2 "..Color.Name.." is going down!"
		...
		if TL then
			...
			TL.Text = a.." ".."2 "..Color.Name.." is going down!"
		end
		delay(0,function()
			Speech(a,"2 "..Color.Name.." is going down!")
		end)
	end
...
Set 2 ]]..Spawned.." down, 5 min break / "..300 - i
			if i > 290 then
				SA.PlaybackSpeed = 1.6
				SA:Play()
			end
