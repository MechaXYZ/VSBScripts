...
NLS([[
    ...
    m.KeyDown:Connect(function(k)
        if k == 'f' and db == false then
            db = true
            local tween = tw:Create(cam, TweenInfo.new(delay), {FieldOfView = currentfov / sd})
            local tween2 = tw:Create(owner.Character.Humanoid, TweenInfo.new(delay), {CameraOffset = owner.Character.Humanoid.CameraOffset + Vector3.new(0, -((currentfov / 70) / sd), 0)})
