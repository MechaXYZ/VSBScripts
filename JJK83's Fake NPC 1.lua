...
function Ready()
	...
	FakeChar.Name = "1NPC"..tostring(Spawned)
...
Set 1 NPC: ]]..Color.Name.." / Health: "..tostring(HP).." / Total: "..Spawned.." / Landed: "..tostring(RegHits)
			end
			Show = Show + 1
		end)
	end
...
Set 1 NPC: ]]..Color.Name.." / Health: "..tostring(HP).." / Total: "..Spawned.." / Landed: "..tostring(RegHits)
		...
		if Hit and Hurt <= 0 then
			...
			if HP <= 0 and Down >= 200 then
				...
				local a = "OW"
				if Tag.Value ~= "" and Tag.Value ~= "1 "..Color.Name then
					a = Tag.Value.." had me..."
				end
				Text = a
				TL.Text = a
				local H = Instance.new("Hint",game.Workspace)
				H.Text = a.." / ".."1 "..Color.Name
	...
	if a ~= "That really hit!" or math.random(1,8) == 1 then
		if BBG and TL then
			...
			TL.TextStrokeColor3 = Color3.new(0,0,0)
			TL.Text = a.." ".."1 "..Color.Name.." is down!"
		end
		if true then
			delay(0,function()
				Speech(a,"1 "..Color.Name.." is down!")
			end)
		end
	end
	Beam.Parent = nil
...
Set 1 ]]..Spawned.." down, 5 min break / "..300 - i.." / Landed: "..tostring(RegHits)
			if i > 290 then
				S.PlaybackSpeed = 1.6
				S:Play()
			end
