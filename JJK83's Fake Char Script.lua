...
function Check(C,B,I,bc,a,U)
	...
	for i = 1,400 do
		if LaserHit == false then
			HC.CFrame = HC.CFrame * CFrame.new(0,0,-2)
			Dist = Dist + 2
			local T = HC:GetTouchingParts()
			for i = 1,#T do
				if T[i] and T[i] ~= U and T[i].Parent then
	...
	HC.Parent = nil
	if HC then
		HC.CFrame = HC.CFrame * CFrame.new(0,0,-3.5)
	end
	if B.Size.X.Scale < 4 then
		B.StudsOffsetWorldSpace = HC.Position
		B.Size = UDim2.new(4,0,4,0)
		I.ImageColor3 = bc.Color
	end
	if a then
		a.Position = Vector3.new(0,0,-Dist)
	end
end
local RegHits = 0
function Shoot(C,bc,f,s,b,B,I,d,wr)
	local Bong = false
	local Confirm = false
	local function CountHit()
		if not Confirm then
			Confirm = true
			RegHits = RegHits + 1
			if owner:FindFirstChild("leaderstats") and owner.leaderstats:FindFirstChild("Hit/-Spawn") and owner.leaderstats["Hit/-Spawn"]:IsA("NumberValue") then
				owner.leaderstats["Hit/-Spawn"].Value = RegHits
			end
			delay(0,function()
				wr.PlaybackSpeed = 1
				wr.TimePosition = 0.2
	...
	if f == true then
		...
		if not d then
			...
			P.Name = "Zap"
			P.Velocity = P.CFrame.lookVector * 160 + Vector3.new(0,50,0)
		end
	...
	for i = 1,Range do
		if P and P.Parent then
			if f == true then
				if d then
					if SA and SA.Parent then
						SA.PlaybackSpeed = 18 - i * 0.06
					end
					if PartA then
						PartA.Position = P.Position
						PartA.Parent = script
						PartA.Parent = nil
					end
				else
	...
	if P and P.Parent then
		...
		if f == true then
			...
			if not d then
				...
				delay(0,function()
					for i = 1,11 do
						if B.Size.X.Scale < 21 then
							B.StudsOffsetWorldSpace = P.Position
							B.Size = UDim2.new(21,0,21,0)
							if i < 11 then
								if i%2 == 1 then
									I.ImageColor3 = Color3.new(1,1,0)
								else
									I.ImageColor3 = Color3.new(0,0,1)
								end
							else
								I.ImageColor3 = bc.Color
							end
...
local DeFunnycat = false
function Ready()
	Spawned = Spawned + 1
	if owner:FindFirstChild("leaderstats") and owner.leaderstats:FindFirstChild("Hit/-Spawn") and owner.leaderstats["Hit/-Spawn"]:IsA("NumberValue") then
		owner.leaderstats["Hit/-Spawn"].Value = -Spawned
	end
	delay(5,function()
		if owner:FindFirstChild("leaderstats") and owner.leaderstats:FindFirstChild("Hit/-Spawn") and owner.leaderstats["Hit/-Spawn"]:IsA("NumberValue") then
			owner.leaderstats["Hit/-Spawn"].Value = RegHits
	...
	FakeChar.Name = "Char"..tostring(Spawned)
	...
	BBG.Adornee = Head
	local CheckA = Instance.new("Attachment",Blast)
	local CheckBeam = Instance.new("Beam",script)
	CheckBeam.Attachment0 = A
	...
	if owner.Neutral == false then
		...
		Image3.ImageColor3 = TeamColor.Color
	else
	...
	local function Healboom()
		for i = 1,30 do
			if SoundC and SoundC.Parent then
				SoundC.PlaybackSpeed = 0.3 + i * 0.03
				SoundC.TimePosition = 0.2
				SoundC:Play()
			end
			wait(0.06)
	...
	local function HealField()
		for i = 1,3 do
			if SoundC then
				SoundC.PlaybackSpeed = 0.5
				SoundC.TimePosition = 0.2
				SoundC:Play()
			end
			wait(0.02)
		...
		for i = 1,9 do
			if SoundA then
				SoundA.PlaybackSpeed = -0.12*(i-6)^2+9
				SoundA.TimePosition = 0.2
				SoundA:Play()
			end
			wait(0.04)
		...
		for i = 1,14 do
			if SoundA then
				SoundA.PlaybackSpeed = -0.03*(i-4)^2+7.5
				SoundA.TimePosition = 0.2
				SoundA:Play()
			end
			wait(0.04)
	...
	local function Grab(part)
		if part and part.Parent then
			GrabPart = part
			GrabWeld = Instance.new("Weld",Blast)
			GrabWeld.Name = "FakeCharGrab"
			GrabWeld.Part0 = Blast
			GrabWeld.Part1 = part
			GrabWeld.C0 = CFrame.new(0,0,-1.5)
		end
	end
	...
	local function Chatted(text,recipient)
		if recipient == nil then
			if string.lower(text) == "normal" or string.lower(text) == "/e normal" then
				Normal = not Normal
				return
			elseif string.lower(text) == "aaaah" or string.lower(text) == "/e aaaah" then
				...
				EmoteTimer = 35
				if string.lower(text) == "/e aaaah" then
					return
				end
			elseif string.lower(text) == "up" or string.lower(text) == "/e up" then
				Up = not Up
				Dn = false
				Disable = false
				Aim = false
				Angle = Vector2.new()
				CheckBeam.Transparency = NumberSequence.new(1)
				CheckA.Position = Vector3.new(0,0,2)
				return
			elseif string.lower(text) == "down" or string.lower(text) == "/e down" then
				Dn = not Dn
				Up = false
				Disable = false
				Aim = false
				...
				CheckA.Position = Vector3.new(0,0,2)
				return
			elseif string.lower(text) == "disable" or string.lower(text) == "/e disable" then
				Disable = not Disable
				Up = false
				Dn = false
				Aim = false
				...
				CheckA.Position = Vector3.new(0,0,2)
				return
			elseif string.lower(text) == "off" or string.lower(text) == "/e off" then
				Off = not Off
				if Off then
					ShipTrail.Lifetime = 0
					ShipTrail.Color = ColorSequence.new(Color3.new())
				...
				return
			elseif string.lower(text) == "damage" or string.lower(text) == "/e damage" then
				Hit = true
				ExtraHit = true
				return
			elseif string.lower(text) == "heal" or string.lower(text) == "/e heal" or string.lower(text) == "heal field!" then
				if HealTimer < 0 and HP > 0 then
					HealTimer = 500
					if TL then
						Text = "HEAL FIELD!"
						TL.Text = "HEAL FIELD!"
					end
					ChatTimer = 75 + string.len("HEAL FIELD!")
					delay(0,function()
						Healboom()
					end)
					delay(0,function()
					...
					local function Touched(hit)
						if hit and hit.Parent then
							...
							if Humanoid and Humanoid.Health < Humanoid.MaxHealth and Humanoid.Health > 0 then
								...
								Bill3.StudsOffsetWorldSpace = hit.Position
					...
					for i = 1,#D do
						if D[i]:IsA("BasePart") and (D[i].Position - P.Position).Magnitude < 21 then
							Touched(D[i])
						end
					end
					for i = 1,10 do
					...
					ChatTimer = 75 + string.len("Not ready.")
				end
				return
			elseif string.lower(text) == "hide" or string.lower(text) == "/e hide" then
				Hide = not Hide
				if TL then
					Text = "???"
					TL.Text = "???"
				end
				ChatTimer = 75 + string.len("???")
				return
			elseif string.lower(text) == "aim" or string.lower(text) == "/e aim" then
				if HP > 0 then
					Aim = not Aim
					Up = false
					Dn = false
					Disable = false
					if Aim then
						local Direction2 = Torso.CFrame.LookVector
						...
						TurnTimer = 0
						CheckBeam.Transparency = NumberSequence.new(0.85)
					else
						Angle = Vector2.new()
						CheckBeam.Transparency = NumberSequence.new(1)
						CheckA.Position = Vector3.new(0,0,2)
					end
					if AimDirect == "Right" then
						Torso.Orientation = Vector3.new(0,270,0)
					elseif AimDirect == "DownRight" then
						Torso.Orientation = Vector3.new(0,225,0)
					elseif AimDirect == "Down" then
						Torso.Orientation = Vector3.new(0,180,0)
					elseif AimDirect == "DownLeft" then
						Torso.Orientation = Vector3.new(0,135,0)
					elseif AimDirect == "Left" then
						Torso.Orientation = Vector3.new(0,90,0)
					elseif AimDirect == "UpLeft" then
						Torso.Orientation = Vector3.new(0,45,0)
					elseif AimDirect == "Up" then
						Torso.Orientation = Vector3.new(0,0,0)
					elseif AimDirect == "UpRight" then
						Torso.Orientation = Vector3.new(0,315,0)
					end
				end
				return
			elseif string.lower(text) == "dance" or string.lower(text) == "/e dance" then
				Up = false
				Dn = false
				Aim = false
				GrabMode = false
			...
			elseif string.lower(text) == "move" or string.lower(text) == "/e move" then
				...
				return
			elseif string.lower(text) == "grab" or string.lower(text) == "/e grab" then
				GrabMode = not GrabMode
				if not GrabMode then
					Release()
				end
				return
			elseif string.lower(text) == "laugh" or string.lower(text) == "/e laugh" then
				...
				LaughTimer = 45
				return
			elseif string.lower(text) == "ahaha" or string.lower(text) == "/e ahaha" then
				if LaughTimer <= 10 then
					delay(0,function()
						AHAHA()
					end)
				...
				LaughTimer = 45
				if string.lower(text) == "/e ahaha" then
					return
				end
			elseif string.lower(text) == "ehaha" or string.lower(text) == "/e ehaha" then
				if LaughTimer <= 10 then
					delay(0,function()
						EHAHA()
					end)
				...
				LaughTimer = 45
				if string.lower(text) == "/e ehaha" then
					return
				end
			elseif string.lower(text) == "yah" or string.lower(text) == "/e yah" then
				delay(0,function()
					YAH()
				end)
				if string.lower(text) == "/e yah" then
					return
				end
			elseif string.lower(text) == "yo" or string.lower(text) == "/e yo" then
				delay(0,function()
					YO()
				end)
				if string.lower(text) == "/e yo" then
					return
				end
			elseif string.lower(text) == "ow" or string.lower(text) == "/e ow" then
				delay(0,function()
					OW()
				end)
				if string.lower(text) == "/e ow" then
					return
				end
			end
	...
	local function Play()
		...
		if SoundA and SoundA.Parent and not Off then
			SoundA.PlaybackSpeed = pitch
			SoundA.TimePosition = 0.12
			SoundA.Playing = true
		end
	end
	while owner and owner.Parent and FakeChar and FakeChar.Parent and Torso and Torso.Parent == FakeChar and Head and Head.Parent == FakeChar and Down > 0 and Torso.Position.Y > game.Workspace.FallenPartsDestroyHeight do
		...
		if Direction == "Idle" and Bump < 0 and Hurt < 0 and HP > 0 and LandTimer >= 0 and not (Up or Dn or Aim or GrabMode) then
		...
		if LandTimer >= 0 and (Torso.Velocity*Vector3.new(1,0,1)).Magnitude > 55 and not Humanoid.Jump then
			OhNo = true
		else
			OhNo = false
		end
		...
		if Disable == false and Humanoid.Jump and ((not Aim and Direction ~= "Idle") or Aim) then
			Bla = true
		end
			...
			if MoveMode then
				if Direction ~= "Idle" then
					...
					if Bla == false and Bump <= 0 then
						Torso.Velocity = Torso.Velocity + Torso.CFrame.LookVector * Movements
					end
				end
			else
		...
		if Aim then
			...
			local AR = false
			local AD = false
			if (D1 == "Up" and (D2 == "UpLeft" or D2 == "Up" or D2 == "UpRight")) or 
				(D1 == "UpRight" and (D2 == "Up" or D2 == "UpRight" or D2 == "Right")) or 
				(D1 == "Right" and (D2 == "UpRight" or D2 == "Right" or D2 == "DownRight")) or 
				(D1 == "DownRight" and (D2 == "Right" or D2 == "DownRight" or D2 == "Down")) or 
				(D1 == "Down" and (D2 == "DownRight" or D2 == "Down" or D2 == "DownLeft")) or 
				(D1 == "DownLeft" and (D2 == "Down" or D2 == "DownLeft" or D2 == "Left")) or 
				(D1 == "Left" and (D2 == "DownLeft" or D2 == "Left" or D2 == "UpLeft")) or 
				(D1 == "UpLeft" and (D2 == "Left" or D2 == "UpLeft" or D2 == "Up")) then
				AU = true
			end
			if (D1 == "UpRight" and (D2 == "UpLeft" or D2 == "Up" or D2 == "UpRight")) or 
				(D1 == "Right" and (D2 == "Up" or D2 == "UpRight" or D2 == "Right")) or 
				(D1 == "DownRight" and (D2 == "UpRight" or D2 == "Right" or D2 == "DownRight")) or 
				(D1 == "Down" and (D2 == "Right" or D2 == "DownRight" or D2 == "Down")) or 
				(D1 == "DownLeft" and (D2 == "DownRight" or D2 == "Down" or D2 == "DownLeft")) or 
				(D1 == "Left" and (D2 == "Down" or D2 == "DownLeft" or D2 == "Left")) or 
				(D1 == "UpLeft" and (D2 == "DownLeft" or D2 == "Left" or D2 == "UpLeft")) or 
				(D1 == "Up" and (D2 == "Left" or D2 == "UpLeft" or D2 == "Up")) then
				AU = true
				AR = true
			end
			if (D1 == "Right" and (D2 == "UpLeft" or D2 == "Up" or D2 == "UpRight")) or 
				(D1 == "DownRight" and (D2 == "Up" or D2 == "UpRight" or D2 == "Right")) or 
				(D1 == "Down" and (D2 == "UpRight" or D2 == "Right" or D2 == "DownRight")) or 
				(D1 == "DownLeft" and (D2 == "Right" or D2 == "DownRight" or D2 == "Down")) or 
				(D1 == "Left" and (D2 == "DownRight" or D2 == "Down" or D2 == "DownLeft")) or 
				(D1 == "UpLeft" and (D2 == "Down" or D2 == "DownLeft" or D2 == "Left")) or 
				(D1 == "Up" and (D2 == "DownLeft" or D2 == "Left" or D2 == "UpLeft")) or 
				(D1 == "UpRight" and (D2 == "Left" or D2 == "UpLeft" or D2 == "Up")) then
				AR = true
			end	
			if (D1 == "DownRight" and (D2 == "UpLeft" or D2 == "Up" or D2 == "UpRight")) or 
				(D1 == "Down" and (D2 == "Up" or D2 == "UpRight" or D2 == "Right")) or 
				(D1 == "DownLeft" and (D2 == "UpRight" or D2 == "Right" or D2 == "DownRight")) or 
				(D1 == "Left" and (D2 == "Right" or D2 == "DownRight" or D2 == "Down")) or 
				(D1 == "UpLeft" and (D2 == "DownRight" or D2 == "Down" or D2 == "DownLeft")) or 
				(D1 == "Up" and (D2 == "Down" or D2 == "DownLeft" or D2 == "Left")) or 
				(D1 == "UpRight" and (D2 == "DownLeft" or D2 == "Left" or D2 == "UpLeft")) or 
				(D1 == "Right" and (D2 == "Left" or D2 == "UpLeft" or D2 == "Up")) then
				AR = true
				AD = true
			end
			if (D1 == "Down" and (D2 == "UpLeft" or D2 == "Up" or D2 == "UpRight")) or 
				(D1 == "DownLeft" and (D2 == "Up" or D2 == "UpRight" or D2 == "Right")) or 
				(D1 == "Left" and (D2 == "UpRight" or D2 == "Right" or D2 == "DownRight")) or 
				(D1 == "UpLeft" and (D2 == "Right" or D2 == "DownRight" or D2 == "Down")) or 
				(D1 == "Up" and (D2 == "DownRight" or D2 == "Down" or D2 == "DownLeft")) or 
				(D1 == "UpRight" and (D2 == "Down" or D2 == "DownLeft" or D2 == "Left")) or 
				(D1 == "Right" and (D2 == "DownLeft" or D2 == "Left" or D2 == "UpLeft")) or 
				(D1 == "DownRight" and (D2 == "Left" or D2 == "UpLeft" or D2 == "Up")) then
				AD = true
			end
			if (D1 == "DownLeft" and (D2 == "UpLeft" or D2 == "Up" or D2 == "UpRight")) or 
				(D1 == "Left" and (D2 == "Up" or D2 == "UpRight" or D2 == "Right")) or 
				(D1 == "UpLeft" and (D2 == "UpRight" or D2 == "Right" or D2 == "DownRight")) or 
				(D1 == "Up" and (D2 == "Right" or D2 == "DownRight" or D2 == "Down")) or 
				(D1 == "UpRight" and (D2 == "DownRight" or D2 == "Down" or D2 == "DownLeft")) or 
				(D1 == "Right" and (D2 == "Down" or D2 == "DownLeft" or D2 == "Left")) or 
				(D1 == "DownRight" and (D2 == "DownLeft" or D2 == "Left" or D2 == "UpLeft")) or 
				(D1 == "Down" and (D2 == "Left" or D2 == "UpLeft" or D2 == "Up")) then
				AD = true
				AL = true
			end
			if (D1 == "Left" and (D2 == "UpLeft" or D2 == "Up" or D2 == "UpRight")) or 
				(D1 == "UpLeft" and (D2 == "Up" or D2 == "UpRight" or D2 == "Right")) or 
				(D1 == "Up" and (D2 == "UpRight" or D2 == "Right" or D2 == "DownRight")) or 
				(D1 == "UpRight" and (D2 == "Right" or D2 == "DownRight" or D2 == "Down")) or 
				(D1 == "Right" and (D2 == "DownRight" or D2 == "Down" or D2 == "DownLeft")) or 
				(D1 == "DownRight" and (D2 == "Down" or D2 == "DownLeft" or D2 == "Left")) or 
				(D1 == "Down" and (D2 == "DownLeft" or D2 == "Left" or D2 == "UpLeft")) or 
				(D1 == "DownLeft" and (D2 == "Left" or D2 == "UpLeft" or D2 == "Up")) then
				AL = true
			end
			if (D1 == "UpLeft" and (D2 == "UpLeft" or D2 == "Up" or D2 == "UpRight")) or 
				(D1 == "Up" and (D2 == "Up" or D2 == "UpRight" or D2 == "Right")) or 
				(D1 == "UpRight" and (D2 == "UpRight" or D2 == "Right" or D2 == "DownRight")) or 
				(D1 == "Right" and (D2 == "Right" or D2 == "DownRight" or D2 == "Down")) or 
				(D1 == "DownRight" and (D2 == "DownRight" or D2 == "Down" or D2 == "DownLeft")) or 
				(D1 == "Down" and (D2 == "Down" or D2 == "DownLeft" or D2 == "Left")) or 
				(D1 == "DownLeft" and (D2 == "DownLeft" or D2 == "Left" or D2 == "UpLeft")) or 
				(D1 == "Left" and (D2 == "Left" or D2 == "UpLeft" or D2 == "Up")) then
				AL = true
				AU = true
			end
			if AL or AU or AR or AD then
				if TurnTimer < 12 then
					TurnTimer = TurnTimer + 1
				end
			elseif TurnTimer > 0 then
				TurnTimer = TurnTimer - 2
			end
			for i = 1,TurnTimer*1.5 do
				if AL and Angle.X > -36 then
					Angle = Angle + Vector2.new(-0.2,0)
				end
				if AU and Angle.Y < 36 then
					Angle = Angle + Vector2.new(0,0.2)
				end
				if AR and Angle.X < 36 then
					Angle = Angle + Vector2.new(0.2,0)
				end
				if AD and Angle.Y > -36 then
					Angle = Angle + Vector2.new(0,-0.2)
				end
			end
			if GrabMode and Angle.Y < -24 then
				Angle = Vector2.new(Angle.X,-24)
			end
		end
		if IdleTimer == 31 then
			local TL = Instance.new("TextLabel",ScreenGui)
			game:GetService("Debris"):AddItem(TL,5)
			TL.AnchorPoint = Vector2.new(0.5,0.5)
			TL.Size = UDim2.new(0.4,0,0.1,0)
			TL.Position = UDim2.new(0.5,0,0.8,0)
			TL.Font = Enum.Font.Code
			TL.BackgroundTransparency = 1
			TL.TextStrokeTransparency = 0
			TL.TextScaled = true
			...
			local Score = "Blasting Grounds by JJK83"
...
Times spawned: ]]..tostring(Spawned)..[[

Attacks landed: ]]..tostring(RegHits)
		end
		if Bump > 35 then
			if Bump == 36 then
				if SoundA and SoundA.Parent then
			...
			if Bump % 2 == 0 then
				...
				if not Aim then
					RightArm.CFrame = Torso.CFrame * CFrame.new(1.5,1.5,0.5,-2.25,0,0,1)
					if RightArm.Parent == FakeChar and Disable == false then
						Blast.CFrame = RightArm.CFrame * CFrame.new(0,-2.25,-0.5,-1,0,0,1)
					else
		...
		elseif HP > 0 then
			if FallTimer >= 38 or EmoteTimer > 0 then
				if IdleTimer <= 0 then
					...
					if RightArm.Parent == FakeChar then
						Blast.CFrame = RightArm.CFrame * CFrame.new(0,-2.25,-0.5,-1,0,0,1)
					else
						Blast.CFrame = Torso.CFrame * CFrame.new(-1.25,1.5,0,0,0,0,1)
					end
			...
			elseif LandTimer < 0 then
				if Angle.X > 16 then
					Head.CFrame = Torso.CFrame * CFrame.new(0,1.5,0,0,-0.3,0,1)
				elseif Angle.X < -16 then
					Head.CFrame = Torso.CFrame * CFrame.new(0,1.5,0,0,0.3,0,1)
				elseif Up or Angle.Y > 16 then
					Head.CFrame = Torso.CFrame * CFrame.new(0,1.4,0.2,0.3,0,0,1)
				elseif Dn or Angle.Y < -16 then
				...
				if Disable then
					RightArm.CFrame = Torso.CFrame * CFrame.new(1.5,1.2,-0.3,4,0,0,1)
				else
					if Up then
						RightArm.CFrame = Torso.CFrame * CFrame.new(1.5,1,0,10000,0,0,1)
					else
			...
			elseif IdleTimer < 32 then
				Head.CFrame = Torso.CFrame * CFrame.new(0,1.5,-0.25,-0.2,-0.2,0,1)
				if (IdleTimer < 7) or ((Off or IdleTimer >= 11) and IdleTimer < 20) then
					LeftArm.CFrame = Torso.CFrame * CFrame.new(-0.7,0.425,-0.9,1.2,0,1.3,1)
				...
				if LaughTimer <= 0 and Angle.X > 16 then
					Head.CFrame = Torso.CFrame * CFrame.new(0,1.5,0,0,-0.3,0,1)
				elseif LaughTimer <= 0 and Angle.X < -16 then
					Head.CFrame = Torso.CFrame * CFrame.new(0,1.5,0,0,0.3,0,1)
				elseif not (LaughTimer > 0 and LaughTimer % 5 > 1) and ((LaughTimer > 0 and LaughTimer % 5 < 2) or Up or Angle.Y > 16) then
					Head.CFrame = Torso.CFrame * CFrame.new(0,1.4,0.2,0.3,0,0,1)
				elseif LaughTimer <= 0 and Dn or Angle.Y < -16 then
					Head.CFrame = Torso.CFrame * CFrame.new(0,1.3,-0.4,-0.4,0,0,1)
				else
					Head.CFrame = Torso.CFrame * CFrame.new(0,1.5,0)
				end
				...
				if Disable then
					if OhNo then
						RightArm.CFrame = Torso.CFrame * CFrame.new(1.5,0.65,0.5,-1.25,0,0,1)
					elseif AnimFrame == 0 then
						RightArm.CFrame = Torso.CFrame * CFrame.new(1.6,0,0,0,0,0.1,1)
					elseif AnimFrame == 1 then
						RightArm.CFrame = Torso.CFrame * CFrame.new(1.5,0,-0.5,0.5,0,0.1,1)
					elseif AnimFrame == 2 then
						RightArm.CFrame = Torso.CFrame * CFrame.new(1.6,0,-0.1,-0.1,0,0.1,1)
					elseif AnimFrame == 3 then
						RightArm.CFrame = Torso.CFrame * CFrame.new(1.5,0,0.5,-0.5,0,0.1,1)
					elseif AnimFrame == 4 then
						RightArm.CFrame = Torso.CFrame * CFrame.new(1.7,0,-0.1,0.1,0,0.2,1)
					end
				else
		...
		if Aim then
			local Offset = math.abs(Angle.X)+math.abs(Angle.Y)
			RightArm.CFrame = Torso.CFrame * CFrame.new(1.5,0.5,-0.5,1,0,0,1)
			Blast.CFrame = Torso.CFrame * CFrame.new(1.5,1,-2.75,0,0,0,1)
			local LV = Blast.CFrame.LookVector
			local RV = Blast.CFrame.RightVector
			local UV = Blast.CFrame.UpVector
			local CF = Blast.CFrame * CFrame.new(Angle.X*4,Angle.Y*4,-160+Offset*2)
			Blast.CFrame = Blast.CFrame * CFrame.new(Angle.X/40,Angle.Y/40,Offset/40)
			Blast.CFrame = CFrame.new(Blast.Position,Vector3.new(CF.X,CF.Y,CF.Z))
			if SightTimer <= 0 then
				Check((Blast.CFrame * CFrame.new(0,0,-6)),Bill,Image,Blast.BrickColor,CheckA,GrabPart)
				SightTimer = 3
			end
		end
		if Direction ~= "Idle" and not Aim and Bla == false then
		...
		if (Fire > 0 or Fire < -30) and not (Fire < -35 and Off) then
			Blast.Material = Enum.Material.Neon
		else
			Blast.Material = Enum.Material.Metal
		end
		...
		for i = 1,20 do
			...
			if GrabMode then
				if Bump < 0 and Hurt < 0 and not Dn and not Disable and Stuck > 19 and GrabPart and GrabPart.Parent and GrabWeld and GrabWeld.Parent then
					local ZOff = -1.5
					if GrabPart.ClassName ~= "CornerWedgePart" and GrabPart.ClassName ~= "UnionOperation" and GrabPart.ClassName ~= "MeshPart" then
						ZOff = -1.5-GrabPart.Size.Z/3
					end
					GrabWeld.C0 = CFrame.new(0,0,ZOff)
					if not GrabPart.Parent:IsA("Workspace") and GrabPart.Parent:FindFirstChild("FakeHumanoid") or GrabPart.Parent:FindFirstChildWhichIsA("Humanoid") then
						local Pos = (Blast.CFrame * CFrame.new(0,0,-1.5)).p
						local Dif = Pos - GrabPart.Position
						local Parts = {}
						local GoFrom = {}
						local D = GrabPart.Parent:GetDescendants()
						for i = 1,#D do
							if D[i]:IsA("BasePart") and D[i].Anchored then
								table.insert(Parts,D[i])
								table.insert(GoFrom,D[i].CFrame)
							end
						end
						for i = 1,#Parts do
							Parts[i].Velocity = Blast.Velocity
							Parts[i].CFrame = GoFrom[i] + Dif
						end
					else
						if GrabPart.Anchored then
							GrabPart.Velocity = Blast.Velocity
							GrabPart.CFrame = Blast.CFrame * CFrame.new(0,0,ZOff)
						end
					end
				else
					Release()
		...
		if Down == 150 and Humanoid.Jump == true then
			HP = 3
			Bump = 0
			Heal = 200
			Bill3.StudsOffsetWorldSpace = Torso.Position
		...
		if Hit and Hurt <= 0 then
			...
			if HP <= 0 and Down >= 200 then
				...
				local a = "OW"
				if Tag.Value ~= "" and Tag.Value ~= owner.Name then
					a = Tag.Value.." had me..."
				end
				Text = a
				TL.Text = a
				...
				if owner:FindFirstChild("leaderstats") and owner.leaderstats:FindFirstChild("Score") and owner.leaderstats.Score:IsA("NumberValue") then
					owner.leaderstats.Score.Value = owner.leaderstats.Score.Value - 2
				end
				local TL = Instance.new("TextLabel",ScreenGui)
				game:GetService("Debris"):AddItem(TL,2.5)
				TL.AnchorPoint = Vector2.new(0.5,0.5)
				TL.Size = UDim2.new(0.4,0,0.1,0)
				TL.Position = UDim2.new(0.5,0,0.8,0)
				TL.Font = Enum.Font.Code
				...
				TL.TextColor3 = Color3.new(0.6,0.6,0)
				TL.TextStrokeColor3 = Color3.new(1,1,0)
				TL.Text = "HOLD JUMP UNTIL RECOVERY"
			end
		else
			Hurt = Hurt - 1
		end
		...
		if Disable == false and Bla and HP > 0 then
			if Fire <= 0 and Blast then
				if Up then
					RightArm.CFrame = Torso.CFrame * CFrame.new(1.5,1,0,10000,0,0,1)
					if RightArm.Parent == FakeChar then
						Blast.CFrame = RightArm.CFrame * CFrame.new(0,-2.25,-0.5,-1,0,0,1)
				...
				local StraightShot = (Up == false and Dn == false and LandTimer < 0 and not Aim)
				...
				for i = 1,#T do
					if T[i] and T[i].Parent and T[i].Parent ~= FakeChar then
						Ping = true
						if T[i].CanCollide or GrabMode then
							Block = true
						end
						if Bill.Size.X.Scale < 16 then
						...
						if Bump < 0 and Hurt < 0 and Stuck > 19 and GrabMode and not Grabbing then
							local H = T[i].Parent:FindFirstChildWhichIsA("Humanoid")
							local h = T[i].Parent:FindFirstChild("FakeHumanoid")
							local Tors = T[i].Parent:FindFirstChild("Torso") or T[i].Parent:FindFirstChild("UpperTorso")
							local Get = false
							local Yes = false
							if h then
								if T[i].Parent.Name ~= "Target" and (not T[i].Parent:FindFirstChild("T") or T[i].Name == "T") then
									Get = true
									Yes = true
								end
							elseif H then
								if not Tors or T[i] == Tors then
									Get = true
									Yes = true
								end
							elseif not T[i]:IsGrounded() then
								Get = true
								if T[i].CanCollide then
									Yes = true
								end
							end
							local JointCheck = true
							local J = T[i]:GetJoints()
							for n = 1,#J do
								if J[n] and J[n].Name == "FakeCharGrab" then
									JointCheck = false
								end
							end
							if Get and (Yes or not Prefer) and T[i].Size.Magnitude < 12 and JointCheck then
								Select = T[i]
								if Yes then
									Prefer = true
								end
							...
							if ExplodingShot then
								...
								delay(0,function()
									for a = 1,11 do
										if Bill.Size.X.Scale < zapsize then
											Bill.StudsOffsetWorldSpace = T[i].Position
											Bill.Size = UDim2.new(zapsize,0,zapsize,0)
											if a < 11 then
												if a%2 == 1 then
													Image.ImageColor3 = Color3.new(1,1,0)
												else
							...
							if Humanoid and not (Humanoid:FindFirstChild("creator") and Humanoid:FindFirstChild("creator"):IsA("ObjectValue") and Humanoid.creator.Value == owner) then
								...
								CountHit()
							elseif Humanoid == nil and T[i]:GetMass() < 120 and not Off then
								delay(0.03,function()
									T[i]:BreakJoints()
								end)
								delay(0,function()
							...
							if T[i].Parent and (not GrabMode or Grabbing) then
								local Tag2 = T[i].Parent:FindFirstChild("FakeHumanoid")
								if Tag2 and owner and owner.Parent then
									Tag2.Value = owner.Name
									CountHit()
				...
				if Ping == true then
					...
					if ExplodingShot and (not GrabMode or Grabbing) then
				...
				delay(0,function()
					...
					if Block == false then
						...
						if ExplodingShot and Dn and LandTimer < 0 then
							FallTimer = 80
							Torso.Velocity = Torso.Velocity + Vector3.new(0,320,0)
							FakeChar:TranslateBy(Vector3.new(0,21,0))
							delay(0,function()
								BANG(Blast.Position)
							end)
							delay(0,function()
						...
						Shoot(Blast.CFrame * CFrame.new(0,0,-6),Blast.BrickColor,ExplodingShot,StraightShot,Beam,Bill2,Image2,Off,SoundB)
					end
				end)
				Fire = 3
			end
		end
		if not Aim and HP > 0 and Humanoid.Jump and LandTimer > 0 and Bump <= 0 and (Direction == "Idle" or Disable == true) then
		...
		if not Off then
			...
			if HealTimer == 0 and HP > 0 then
				Text = "Heal ready!"
				TL.Text = "Heal ready!"
				ChatTimer = 75 + string.len("Heal ready!")
			end
		end
		Bump = Bump - 1
		...
		if Fire > 1 and not GrabPart then
			Blast.Name = "Blade"
			...
			if LastSuper then
				...
				if not GrabMode then
					if not Off then
						...
						Blast.Velocity = Blast.CFrame.lookVector * 400 + Vector3.new(0,200,0)
					end
				else
					if not Off then
						Blast.Velocity = Blast.CFrame.lookVector * 850 + Vector3.new(0,25,0)
					else
						Blast.Velocity = Blast.CFrame.lookVector * 425 + Vector3.new(0,25,0)
					end
				end
			else
		...
		if TL then
			if HP == 5 then
				...
				B7.BackgroundColor3 = Color3.new(0,1,1)
			elseif HP == 4 then
				TL.TextStrokeColor3 = Color3.new(0,1,0)
				B7.BackgroundColor3 = Color3.new(0,1,0)
			elseif HP == 3 then
				TL.TextStrokeColor3 = Color3.new(1,1,0)
				B7.BackgroundColor3 = Color3.new(1,1,0)
			elseif HP == 2 then
				TL.TextStrokeColor3 = Color3.new(1,0,1)
				B7.BackgroundColor3 = Color3.new(1,0,1)
			elseif HP == 1 then
				TL.TextStrokeColor3 = Color3.new(1,0,0)
				B7.BackgroundColor3 = Color3.new(1,0,0)
			else
		...
		if owner and owner.Parent and owner.Neutral == false then
			...
			Torso.BrickColor = Substitute
			if Blast then
				Blast.BrickColor = Substitute
			end
		...
		if HP > 0 and BlinkTimer > 1 and Bump <= 0 then
			if not (LaughTimer > 0 and LaughTimer % 5 < 2) and ((LaughTimer > 0 and LaughTimer % 5 > 1) or Up or Angle.Y > 16) then
			...
			elseif (LaughTimer > 0 and LaughTimer % 5 < 2) or Dn or Angle.Y < -16 then
			...
			if Fire > 0 then
				...
				RD.Color3 = Torso.Color
			elseif Fire < -30 and Fire % 2 == 0 and Disable == false then
				LD.Color3 = Torso.Color
			elseif Fire < -30 and Disable == false then
			...
			if Down >= 150 and Down % 2 == 0 and Humanoid.Jump and HP <= 0 then
				LD.Color3 = Color3.new(1,1,1)
				RD.Color3 = Color3.new(1,1,1)
			end
			Left.CFrame = Head.CFrame * CFrame.new(-0.25,0.0625,-0.5)
		...
		elseif Bump > 35 and Bump%2 == 0 then
			...
			if Dn then
				B4.BackgroundColor3 = Color3.new(0,1,0)
			else
				B4.BackgroundColor3 = Color3.new(1,0,0)
			end
			if Disable then
				B5.BackgroundColor3 = Color3.new(0,1,0)
			else
				B5.BackgroundColor3 = Color3.new(1,0,0)
			end
			if Off then
				B6.BackgroundColor3 = Color3.new(0,1,0)
			else
				B6.BackgroundColor3 = Color3.new(1,0,0)
			end
			if HealTimer < 0 then
				B8.BackgroundColor3 = Color3.new(0,1,1)
			else
				B8.BackgroundColor3 = Color3.new(1,0,1)
			end
			if Hide then
				B9.BackgroundColor3 = Color3.new(0,1,0)
			else
				B9.BackgroundColor3 = Color3.new(1,0,0)
			end
			if Aim then
				B10.BackgroundColor3 = Color3.new(0,1,0)
			else
				B10.BackgroundColor3 = Color3.new(1,0,0)
			end
			if MoveMode then
				B11.BackgroundColor3 = Color3.new(0,1,0)
			else
				B11.BackgroundColor3 = Color3.new(1,0,0)
			end
			if GrabMode then
				if GrabPart then
					B12.BackgroundColor3 = Color3.new(1,1,0)
				else
					B12.BackgroundColor3 = Color3.new(0,1,0)
				end
			else
				B12.BackgroundColor3 = Color3.new(1,0,0)
			end
			B13.BackgroundColor3 = Substitute.Color
			B14.BackgroundColor3 = Brightness.Color
		end
		Hit = false
		ExtraHit = false
		Bla = false
		...
		if CheckBeam then
			CheckBeam.Color = ColorSequence.new(Blast.Color)
		end
		wait(0.04)
	end
	LD.Color3 = Torso.Color
	...
	if a ~= "That really hit!" or math.random(1,8) == 1 then
		...
		if BBG and TL then
			...
			TL.TextStrokeColor3 = Color3.new(0,0,0)
			TL.Text = a.." "..owner.Name.." is down!"
		end
		if owner and owner.Parent then
			delay(0,function()
				Speech(a,owner.Name.." is down!")
			end)
		end
	end
	local Match = game:GetService("Players"):FindFirstChild(Tag.Value)
	if Match and Match:IsA("Player") then
		if Match:FindFirstChild("leaderstats") and Match.leaderstats:FindFirstChild("Score") and Match.leaderstats.Score:IsA("NumberValue") then
			if owner == Match then
				Match.leaderstats.Score.Value = Match.leaderstats.Score.Value - 1
			else
				Match.leaderstats.Score.Value = Match.leaderstats.Score.Value + 2
			end
		end
	...
	if owner:FindFirstChild("leaderstats") and owner.leaderstats:FindFirstChild("Score") and owner.leaderstats.Score:IsA("NumberValue") then
		owner.leaderstats.Score.Value = owner.leaderstats.Score.Value - 1
	end
	if owner:FindFirstChild("leaderstats") and owner.leaderstats:FindFirstChild("Health") and owner.leaderstats.Health:IsA("NumberValue") then
		owner.leaderstats.Health.Value = HP
	end
...
Spawning... (]]..tostring(Spawned+1)..[[)
Attacks landed: ]]..tostring(RegHits)
		if not DeFunnycat and (RegHits%80 == 0 and RegHits > 0) then
			local TL = Instance.new("TextLabel",ScreenGui)
			TL.AnchorPoint = Vector2.new(0.5,0.5)
			TL.Size = UDim2.new(0.4,0,0.1,0)
			...
			TL.TextStrokeTransparency = 0
			TL.TextScaled = true
			TL.TextColor3 = Color3.new(0,0.6,0)
			TL.TextStrokeColor3 = Color3.new(0,1,0)
			TL.Text = [[TRY HOLDING JUMP FOR HIDDEN FEATURE
You have an attacks landed count of a multiple of 80]]
		end
	end
	Beam.Parent = nil
	CheckBeam.Parent = nil
...
function HiddenFeature()
	...
	local RD = Instance.new("Decal",Right)
	RD.Texture = "rbxasset://textures/CollisionGroupsEditor/unchecked.png"
	local LeftEar1 = Instance.new("Part",FakeChar)
	LeftEar1.Name = "LE1"
	LeftEar1.Size = Vector3.new(0.125,0.375,0.125)
	LeftEar1.Anchored = true
	LeftEar1.Material = Enum.Material.Neon
	LeftEar1.Locked = true
	LeftEar1.CanCollide = false
	local LeftEar2 = Instance.new("Part",FakeChar)
	LeftEar2.Name = "LE2"
	LeftEar2.Size = Vector3.new(0.125,0.25,0.125)
	LeftEar2.Anchored = true
	LeftEar2.Material = Enum.Material.Neon
	LeftEar2.Locked = true
	LeftEar2.CanCollide = false
	local LeftEar3 = Instance.new("Part",FakeChar)
	LeftEar3.Name = "LE3"
	LeftEar3.Size = Vector3.new(0.125,0.125,0.125)
	LeftEar3.Anchored = true
	LeftEar3.Material = Enum.Material.Neon
	LeftEar3.Locked = true
	LeftEar3.CanCollide = false
	local RightEar1 = Instance.new("Part",FakeChar)
	RightEar1.Name = "RE1"
	RightEar1.Size = Vector3.new(0.125,0.375,0.125)
	RightEar1.Anchored = true
	RightEar1.Material = Enum.Material.Neon
	RightEar1.Locked = true
	RightEar1.CanCollide = false
	local RightEar2 = Instance.new("Part",FakeChar)
	RightEar2.Name = "RE2"
	RightEar2.Size = Vector3.new(0.125,0.25,0.125)
	RightEar2.Anchored = true
	RightEar2.Material = Enum.Material.Neon
	RightEar2.Locked = true
	RightEar2.CanCollide = false
	local RightEar3 = Instance.new("Part",FakeChar)
	RightEar3.Name = "RE3"
	RightEar3.Size = Vector3.new(0.125,0.125,0.125)
	RightEar3.Anchored = true
	RightEar3.Material = Enum.Material.Neon
	RightEar3.Locked = true
	RightEar3.CanCollide = false
	local Tail = Instance.new("Part",FakeChar)
	Tail.Name = "TA"
	Tail.Size = Vector3.new(0.5,0.5,2.5)
	Tail.Anchored = true
	...
	Head.CFrame = Torso.CFrame * CFrame.new(0,1,-0.5)
	LeftArm.CFrame = Torso.CFrame * CFrame.new(-1,0.2,-1.25,1.5,0.6,0,1)
	RightArm.CFrame = Torso.CFrame * CFrame.new(1,0.2,-1.25,1.5,-0.6,0,1)
	LeftLeg.CFrame = Torso.CFrame * CFrame.new(-0.5,-1.1,0.5,0.3,0.2,0,1)
	RightLeg.CFrame = Torso.CFrame * CFrame.new(0.5,-1,0.45,0.5,-0.2,0,1)
	Tail.CFrame = Torso.CFrame * CFrame.new(0,1.5,1,-1,0,0,1)
	Blast.CFrame = Torso.CFrame * CFrame.new(1.125,1,-2.25,0,0,0,1)
	Left.CFrame = Head.CFrame * CFrame.new(-0.1875,0.1875,-0.5)
	Right.CFrame = Head.CFrame * CFrame.new(0.1875,0.1875,-0.5)
	LeftEar1.CFrame = Head.CFrame*CFrame.new(-0.4375,0.6875,-0.0625)
	LeftEar2.CFrame = Head.CFrame*CFrame.new(-0.315,0.625,-0.0625)
	LeftEar3.CFrame = Head.CFrame*CFrame.new(-0.1925,0.5625,-0.0625)
	RightEar1.CFrame = Head.CFrame*CFrame.new(0.4375,0.6875,-0.0625)
	RightEar2.CFrame = Head.CFrame*CFrame.new(0.315,0.625,-0.0625)
	RightEar3.CFrame = Head.CFrame*CFrame.new(0.1925,0.5625,-0.0625)
	delay(0,function()
		LaunchSound(Torso.Position)
	end)
	local Humanoid = owner.Character:FindFirstChildWhichIsA("Humanoid")
	...
	LeftLeg.Material = Enum.Material.DiamondPlate
	RightLeg.Material = Enum.Material.DiamondPlate
	LeftEar1.Material = Enum.Material.Metal
	LeftEar2.Material = Enum.Material.Metal
	LeftEar3.Material = Enum.Material.Metal
	RightEar1.Material = Enum.Material.Metal
	RightEar2.Material = Enum.Material.Metal
	RightEar3.Material = Enum.Material.Metal
	...
	local function Chatted(text,recipient)
		if recipient == nil then
			...
			elseif string.lower(text) == "aim" or string.lower(text) == "/e aim" then
				...
				return
			elseif string.lower(text) == "rest" or string.lower(text) == "/e rest" or string.lower(text) == "dance" or string.lower(text) == "/e dance" then
				Up = false
				Dn = false
				Aim = false
				GrabMode = false
...
Attacks landed: ]]..tostring(RegHits)
		...
		if Bump > 35 then
			...
			if Bump % 2 == 0 then
				...
				Head.CFrame = Torso.CFrame * CFrame.new(0,1,-0.5)
				LeftArm.CFrame = Torso.CFrame * CFrame.new(-0.8,-0.5,-1.5,1,0.2,-0.2,1)
				RightArm.CFrame = Torso.CFrame * CFrame.new(0.8,-0.5,-1.5,1,-0.2,0.2,1)
				LeftLeg.CFrame = Torso.CFrame * CFrame.new(-0.6,-0.5,0.5,1,0.05,-0.05,1)
				RightLeg.CFrame = Torso.CFrame * CFrame.new(0.6,-0.5,0.5,1,-0.05,0.05,1)
				Tail.CFrame = Torso.CFrame * CFrame.new(0,-0.12,1.8,0.28,0,0,1)
				if Bill.Size.X.Scale < 4 then
					Bill.StudsOffsetWorldSpace = Torso.Position
					Bill.Size = UDim2.new(4,0,4,0)
					if randzap == 1 then
		...
		elseif Bump > 13 or Hurt > 0 or (Hurt > -6 and HP <= 0) then
			if Hurt % 2 == 0 then
				...
				Head.CFrame = Torso.CFrame * CFrame.new(0,1,-0.5)
				LeftArm.CFrame = Torso.CFrame * CFrame.new(-1,0.2,-1.25,1.9,0.6,0,1)
				RightArm.CFrame = Torso.CFrame * CFrame.new(1,0.2,-1.25,1.2,-0.6,0,1)
				LeftLeg.CFrame = Torso.CFrame * CFrame.new(-0.5,-1,0.35,0.45,0.2,0,1)
				RightLeg.CFrame = Torso.CFrame * CFrame.new(0.5,-1,0.45,0.3,-0.2,0,1)
				Tail.CFrame = Torso.CFrame * CFrame.new(0,1.475,1.15,-0.85,0,0,1)
			end
		elseif HP > 0 then
			if FallTimer >= 38 or EmoteTimer > 0 then
				if IdleTimer <= 0 then
				...
				if (FallTimer >= 38 and FallTimer % 2 == 0) or (FallTimer < 38 and EmoteTimer % 2 == 0) then
					Head.CFrame = Torso.CFrame * CFrame.new(0,1,-0.5)
					LeftArm.CFrame = Torso.CFrame * CFrame.new(-0.5,-1,-1.25,0.1,0,0,1)
					RightArm.CFrame = Torso.CFrame * CFrame.new(0.5,-1,-1.25,0.15,0,0,1)
					LeftLeg.CFrame = Torso.CFrame * CFrame.new(-0.5,-0.2,1.45,-1.5,0,0,1)
					RightLeg.CFrame = Torso.CFrame * CFrame.new(0.5,-0.1,1.4,-2.6,0,0,1)
					Tail.CFrame = Torso.CFrame * CFrame.new(0,1.475,0.85,-1.2,0,0,1)
				else
					Head.CFrame = Torso.CFrame * CFrame.new(0,1,-0.5)
					LeftArm.CFrame = Torso.CFrame * CFrame.new(-0.5,-1,-1.25,0.15,0,0,1)
					RightArm.CFrame = Torso.CFrame * CFrame.new(0.5,-1,-1.25,0.225,0,0,1)
					LeftLeg.CFrame = Torso.CFrame * CFrame.new(-0.5,-0.2,1.45,-1.7,0,0,1)
					RightLeg.CFrame = Torso.CFrame * CFrame.new(0.5,-0.1,1.4,-2.3,0,0,1)
					Tail.CFrame = Torso.CFrame * CFrame.new(0,1.46,0.765,-1.3,0,0,1)
				end
			elseif Bump > 0 then
				Head.CFrame = Torso.CFrame * CFrame.new(0,1,-0.5)
				LeftArm.CFrame = Torso.CFrame * CFrame.new(-0.8,-0.5,-1.5,1,0.2,-0.2,1)
			...
			elseif LandTimer < 0 then
				if Angle.X > 16 then
					Head.CFrame = Torso.CFrame * CFrame.new(0,1,-0.5,0,-0.3,0,1)
				elseif Angle.X < -16 then
					Head.CFrame = Torso.CFrame * CFrame.new(0,1,-0.5,0,0.3,0,1)
				elseif Up or Angle.Y > 16 then
					Head.CFrame = Torso.CFrame * CFrame.new(0,0.9,-0.3,0.3,0,0,1)
				elseif Dn or Angle.Y < -16 then
					Head.CFrame = Torso.CFrame * CFrame.new(0,0.8,-0.9,-0.4,0,0,1)
				else
					Head.CFrame = Torso.CFrame * CFrame.new(0,1,-0.5)
				end
				LeftArm.CFrame = Torso.CFrame * CFrame.new(-0.5,-0.6,-1.6,0.7,0,0,1)
				RightArm.CFrame = Torso.CFrame * CFrame.new(0.5,-0.6,-1.6,0.8,0,0,1)
				LeftLeg.CFrame = Torso.CFrame * CFrame.new(-0.7,-0.5,1.3,-0.8,-0.2,0,1)
				RightLeg.CFrame = Torso.CFrame * CFrame.new(0.7,-0.5,1.3,-0.9,0.2,0,1)
				Tail.CFrame = Torso.CFrame * CFrame.new(0,1.05,1.95,-0.3,0,0,1)
			elseif IdleTimer < 0 then
				if IdleTimer < -3 then
					Play()
					IdleTimer = 0
				end
				Head.CFrame = Torso.CFrame * CFrame.new(0.02,0.4,-1.4,-0.5,-0.05,0,1)
				LeftArm.CFrame = Torso.CFrame * CFrame.new(-0.8,-0.5,-1.5,1,0.2,-0.2,1)
				RightArm.CFrame = Torso.CFrame * CFrame.new(0.8,-0.5,-1.5,1,-0.2,0.2,1)
				LeftLeg.CFrame = Torso.CFrame * CFrame.new(-0.6,-0.5,0.5,1,0.05,-0.05,1)
				RightLeg.CFrame = Torso.CFrame * CFrame.new(0.6,-0.5,0.5,1,-0.05,0.05,1)
			...
			elseif IdleTimer < 32 then
				...
				elseif IdleTimer < 26 then
					...
					Head.CFrame = Torso.CFrame * CFrame.new(0,1,-0.5)
					LeftArm.CFrame = Torso.CFrame * CFrame.new(-0.5,-0.8,-1.5,0.6,0.05,0,1)
					RightArm.CFrame = Torso.CFrame * CFrame.new(0.5,-0.8,-1.5,0.6,-0.05,0,1)
					LeftLeg.CFrame = Torso.CFrame * CFrame.new(-0.5,-0.8,0.2,0.6,0.2,0,1)
					RightLeg.CFrame = Torso.CFrame * CFrame.new(0.5,-0.8,0.2,0.6,-0.2,0,1)
					Tail.CFrame = Torso.CFrame * CFrame.new(0,0.5,2,0,0,0,1)
				end
			else
				if LaughTimer <= 0 and Angle.X > 16 then
					Head.CFrame = Torso.CFrame * CFrame.new(0,1,-0.5,0,-0.3,0,1)
				...
				if OhNo then
					LeftArm.CFrame = Torso.CFrame * CFrame.new(-0.65,-0.6,-1.6,0.8,0.15,-0.15,1)
					RightArm.CFrame = Torso.CFrame * CFrame.new(0.65,-0.6,-1.6,0.8,-0.15,0.15,1)
					LeftLeg.CFrame = Torso.CFrame * CFrame.new(-0.8,-0.5,1.5,-0.8,-0.3,-0.3,1)
					RightLeg.CFrame = Torso.CFrame * CFrame.new(0.8,-0.5,1.5,-0.8,0.3,0.3,1)
					Tail.CFrame = Torso.CFrame * CFrame.new(0,1.05,1.95,-0.3,0,0,1)
				elseif AnimFrame == 0 then
					LeftArm.CFrame = Torso.CFrame * CFrame.new(-0.5,-1.25,-1,0,0.05,0,1)
					RightArm.CFrame = Torso.CFrame * CFrame.new(0.5,-1.25,-1,0,-0.05,0,1)
					LeftLeg.CFrame = Torso.CFrame * CFrame.new(-0.65,-1.2,1,0,0,-0.05,1)
					RightLeg.CFrame = Torso.CFrame * CFrame.new(0.65,-1.2,1,0,0,0.05,1)
					Tail.CFrame = Torso.CFrame * CFrame.new(0,1.465,1.2,1.25,0,0,1)
				elseif AnimFrame == 1 then
					LeftArm.CFrame = Torso.CFrame * CFrame.new(-0.55,-1,-1.3,0.35,0.03,0,1)
					RightArm.CFrame = Torso.CFrame * CFrame.new(0.55,-1,-1.3,0.35,-0.03,0,1)
					LeftLeg.CFrame = Torso.CFrame * CFrame.new(-0.65,-1.1,1.1,-0.35,0.15,-0.16,1)
					RightLeg.CFrame = Torso.CFrame * CFrame.new(0.65,-1.1,1.1,-0.35,-0.15,0.16,1)
					Tail.CFrame = Torso.CFrame * CFrame.new(-0.04,0.15,1.95,0.15,-0.03,0,1)
				elseif AnimFrame == 2 then
					LeftArm.CFrame = Torso.CFrame * CFrame.new(-0.5,-1.2,-0.7,-0.2,0.2,0,1)
					RightArm.CFrame = Torso.CFrame * CFrame.new(0.5,-1.2,-0.7,-0.2,-0.2,0,1)
					LeftLeg.CFrame = Torso.CFrame * CFrame.new(-0.7,-0.9,0.3,0.5,0.2,-0.2,1)
					RightLeg.CFrame = Torso.CFrame * CFrame.new(0.7,-0.9,0.3,0.5,-0.2,0.2,1)
					Tail.CFrame = Torso.CFrame * CFrame.new(0.175,0.35,2,0.05,0.07,0,1)
				elseif AnimFrame == 3 then
					LeftArm.CFrame = Torso.CFrame * CFrame.new(-0.55,-0.8,-0.9,-0.175,0.1,0,1)
					RightArm.CFrame = Torso.CFrame * CFrame.new(0.55,-0.8,-0.9,-0.175,-0.1,0,1)
					LeftLeg.CFrame = Torso.CFrame * CFrame.new(-0.55,-1.2,0.8,0.1,0.15,-0.12,1)
					RightLeg.CFrame = Torso.CFrame * CFrame.new(0.55,-1.2,0.8,0.1,-0.15,0.12,1)
					Tail.CFrame = Torso.CFrame * CFrame.new(0.08,0.6,2,-0.12,0.02,0,1)
				elseif AnimFrame == 4 then
					LeftArm.CFrame = Torso.CFrame * CFrame.new(-0.525,-0.75,-1.5,0.8,0.05,0,1)
					RightArm.CFrame = Torso.CFrame * CFrame.new(0.525,-0.75,-1.5,0.8,-0.05,0,1)
					LeftLeg.CFrame = Torso.CFrame * CFrame.new(-0.5,-0.9,1.4,-0.45,0.07,-0.07,1)
					RightLeg.CFrame = Torso.CFrame * CFrame.new(0.5,-0.9,1.4,-0.45,-0.07,0.07,1)
					Tail.CFrame = Torso.CFrame * CFrame.new(-0.08,0.3,2,0.06,-0.06,0,1)
				end
				if Fire > 0 then
					Tail.CFrame = Torso.CFrame * CFrame.new(0,1.35,1.6,1.9,0,0,1)
				end
			end
		else
			Head.CFrame = Torso.CFrame * CFrame.new(0,0.4,-1.4,-0.65,0,0,1)
			LeftArm.CFrame = Torso.CFrame * CFrame.new(-0.8,-0.5,-1.5,1,0.2,-0.2,1)
			RightArm.CFrame = Torso.CFrame * CFrame.new(0.8,-0.5,-1.5,1,-0.2,0.2,1)
			Blast.CFrame = Torso.CFrame * CFrame.new(1.125,1,-2.25,0,0,0,1)
			LeftLeg.CFrame = Torso.CFrame * CFrame.new(-0.6,-0.5,1.5,-1,0.05,-0.05,1)
			RightLeg.CFrame = Torso.CFrame * CFrame.new(0.6,-0.5,1.5,-1,-0.05,0.05,1)
			Tail.CFrame = Torso.CFrame * CFrame.new(0,0.25,2,0.1,0,0,1)
		end
		if HP > 0 and (Up or Dn or Aim) then
			if Up then
				Blast.CFrame = Torso.CFrame * CFrame.new(1.125,2,-1.25,1,0,0,1)
			elseif Dn then
				Blast.CFrame = Torso.CFrame * CFrame.new(1.125,-1,-1.25,-1,0,0,1)
			elseif Aim then
				local Offset = math.abs(Angle.X)+math.abs(Angle.Y)
				Blast.CFrame = Torso.CFrame * CFrame.new(1.125,1,-2.25,0,0,0,1)
				local LV = Blast.CFrame.LookVector
		...
		for i = 1,20 do
			...
			if HeadCollided and not (Bump > 0 or Hurt > 0 or HP <= 0 or FallTimer > 38 or EmoteTimer > 0) then
				LeftLeg.CFrame = Torso.CFrame * CFrame.new(-0.5,-1.25,1)
				RightLeg.CFrame = Torso.CFrame * CFrame.new(0.5,-1.25,1)
			end
			local LT = LeftLeg:GetTouchingParts()
			local LAT = LeftArm:GetTouchingParts()
			for i = 1,#LAT do
				table.insert(LT,LAT[i])
			end
			for i = 1,#LT do
				if LT[i] and LT[i].Parent and LT[i].Parent ~= FakeChar then
					local HitTag = LT[i]:FindFirstChild("HitTag")
				...
				if LT[i] and LT[i].Parent ~= FakeChar and ((LT[i].CanCollide or NameMatch(LT[i])) or (LT[i].Size.Magnitude < 10 and LT[i]:FindFirstChildWhichIsA("TouchTransmitter") and LT[i].Name ~= "T")) then
					...
					if LT[i].CanCollide then
						...
						if LandTimer < 0 then
							Col = true
							if Bill2.Size.X.Scale < 8 then
								Bill2.StudsOffsetWorldSpace = Torso.Position + Vector3.new(0,-1.8,0)
								Bill2.Size = UDim2.new(8,0,8,0)
								Image2.ImageColor3 = LastStep.Color
							end
						end
			...
			local RT = RightLeg:GetTouchingParts()
			local RAT = RightArm:GetTouchingParts()
			for i = 1,#RAT do
				table.insert(RT,RAT[i])
			end
			for i = 1,#RT do
				if RT[i] and RT[i].Parent and RT[i].Parent ~= FakeChar then
					local HitTag = RT[i]:FindFirstChild("HitTag")
		...
		if OhNo then
			...
			Stopped = false
			if Bill2.Size.X.Scale < 6 then
				Bill2.StudsOffsetWorldSpace = Torso.Position + Vector3.new(0,-0.8,0)
				Bill2.Size = UDim2.new(8,0,8,0)
				Image2.ImageColor3 = LastStep.Color
			end
		elseif AnimFrame == 3 and LandTimer > 0 and Bump <= 0 then
			if Sliding then
				SoundC.PlaybackSpeed = 2.5
			else
				SoundC.PlaybackSpeed = 1.2
			...
			Stopped = false
			if Bill2.Size.X.Scale <= 4 then
				Bill2.StudsOffsetWorldSpace = Torso.Position + Vector3.new(0,-1.8,0) + Torso.CFrame.LookVector*0.5
				Bill2.Size = UDim2.new(4,0,4,0)
				Image2.ImageColor3 = LastStep.Color
			end
		elseif AnimFrame == 4 and LandTimer > 0 and Bump <= 0 then
			...
			Stopped = false
			if Bill2.Size.X.Scale <= 4 then
				Bill2.StudsOffsetWorldSpace = Torso.Position + Vector3.new(0,-1.8,0) + Torso.CFrame.LookVector*-1
				Bill2.Size = UDim2.new(4,0,4,0)
				Image2.ImageColor3 = LastStep.Color
			end
		elseif Stopped == false then
		...
		if Hit and Hurt <= 0 then
			...
			if HP <= 0 and Down >= 200 then
				...
				RightArm.Material = Enum.Material.CorrodedMetal
				LeftLeg.Material = Enum.Material.CorrodedMetal
				RightEar1.Material = Enum.Material.CorrodedMetal
				RightEar2.Material = Enum.Material.CorrodedMetal
				RightEar3.Material = Enum.Material.CorrodedMetal
	...
	if owner and owner.Parent then
		if ((RegHits%80 == 0 and RegHits > 0) or DeFunnycat) and Humanoid.Jump then
			if not DeFunnycat then
				...
				TL.TextColor3 = Color3.new(0,0.6,0)
				TL.TextStrokeColor3 = Color3.new(0,1,0)
				TL.Text = [[You will still be able to do this hidden feature later
No need to wait for 80 more hits]]
				DeFunnycat = true
				local Huh = "Huh, wait-- HEY,"
				local How = "who's been taking apart my robots and rearranging them as cats?!"
				local Chooses = math.random(1,4)
				if Chooses == 1 then
					Huh = "NOOOOOOOO!"
					How = "You aren't supposed to hold jump when you respawn with a hit count of a multiple of 80!"
				elseif Chooses == 2 then
					Huh = "Nice work, you had just discovered my"
					How = "joke edit."
				elseif Chooses == 3 then
					Huh = "Not fair!"
					How = "That cat animation pack makes them harder to hit!"
				end
				local H = Instance.new("Hint",game.Workspace)
				H.Text = Huh.." "..How
				game.Debris:AddItem(H,4)
		...
		if math.random(1,2) == 1 then
			local H = Instance.new("Hint",game.Workspace)
			H.Text = "He's going down! / "..owner.Name..", "..Spawned.." spawns, "..RegHits.." landed"
			game.Debris:AddItem(H,4)
			Speech("He's going down!","")
		else
			local H = Instance.new("Hint",game.Workspace)
			H.Text = "This doesn't look good! / "..owner.Name..", "..Spawned.." spawns, "..RegHits.." landed"
			game.Debris:AddItem(H,4)
			Speech("","This doesn't look good!")
		end
		break
	end
end
